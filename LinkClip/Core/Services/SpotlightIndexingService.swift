...
final class SpotlightIndexingService: SpotlightIndexing {
    ...
    private func makeSearchableItem(from link: LinkItem) -> CSSearchableItem {
        ...
        attr.keywords = makeKeywords(fromTitle: link.title,
                                     ...
                                     urlString: link.url,
                                     categoryNames: link.categories?.map { $0.name })
        ...
        // 여러 카테고리가 있는 경우 첫 번째 카테고리의 ID를 사용하거나, 없으면 "uncategorized"
        let domainId = link.categories?.first?.id.uuidString ?? "uncategorized"
    ...
    private func makeSearchableItem(from entry: SpotlightEntry) -> CSSearchableItem {
        ...
        let domainId = entry.categoryIds?.first?.uuidString ?? "uncategorized"
    ...
    private func makeKeywords(fromTitle title: String,
        ...
        if let categoryNames {
            categoryNames.filter { !$0.isEmpty }.forEach { words.insert($0) }
        }
